...
class UpdateCoupon
{
    ...
    public function handle(Company $company, Coupon $coupon, array $attributes, ?User $actor = null): Coupon
    {
        ...
        if (Coupon::query()->where('code', $code)->whereKeyNot($coupon->id)->exists()) {
            throw new InvalidArgumentException('Ya existe otro cupon con ese codigo.');
        }

        $this->assertTargetsExist($planIds, $bundleIds);

        return DB::transaction(function () use ($company, $actor, $attributes, $bundleIds, $code, $coupon, $discountType, $discountValue, $name, $planIds, $status) {
            $before = $coupon->fresh(['plans', 'bundles']);
            ...
            $after = $coupon->fresh(['plans', 'bundles']);
