...
class UpdateDraftPurchase
{
    ...
    public function handle(Company $company, Purchase $purchase, array $attributes): Purchase
    {
        ...
        if (! $this->isEditableDraft($purchase)) {
            throw new InvalidArgumentException('Solo se pueden editar compras en borrador antes del posting definitivo.');
        }
        ...
        return DB::transaction(function () use ($company, $purchase, $branch, $warehouse, $supplier, $attributes, $calculatedLines, $totals, $hasItems) {
            $purchase = Purchase::query()
                ->lockForUpdate()
                ->with(['payableMovements'])
            ...
            $purchase = $purchase->fresh(['branch', 'warehouse', 'payableMovements']);
            $this->auditLogger->logUpdated($company, 'purchase.draft_updated', $beforePurchase, $purchase);

            return $purchase;
        });
    }

    protected function isEditableDraft(Purchase $purchase): bool
    {
        return $purchase->status === PurchaseStatus::Draft->value
            && $purchase->posted_to_inventory_at === null
            && $purchase->returned_from_inventory_at === null
            && bccomp((string) $purchase->amount_paid, '0.00', 2) === 0
            && bccomp((string) $purchase->balance_due, '0.00', 2) === 0
            && $purchase->payableMovements->isEmpty();
    }
