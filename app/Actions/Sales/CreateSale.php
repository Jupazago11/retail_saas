...
class CreateSale
{
    public function __construct(
        ...
        protected SaleDocumentNumberGenerator $saleDocumentNumberGenerator,
        protected AuditLogger $auditLogger,
    ) {}

    public function handle(Company $company, array $attributes): Sale
    {
        ...
        $loyaltyRedemption = $this->resolveLoyaltyRedemption(
            $company,
            $loyaltyAccount,
        ...
        return DB::transaction(function () use ($company, $branch, $warehouse, $cashRegister, $user, $saleType, $customer, $creditAccount, $loyaltyAccount, $attributes, $calculatedLines, $totals, $promotionPayload, $loyaltyRedemption) {
            $documentData = $this->saleDocumentNumberGenerator->nextForCompany($company);

            $sale = Sale::query()->create([
                'company_id' => $company->id,
                'branch_id' => $branch->id,
                'warehouse_id' => $warehouse->id,
                'document_sequence' => $documentData['document_sequence'],
            ...
            $sale = $sale->fresh(['items.product', 'items.presentation', 'items.variant', 'branch', 'warehouse', 'cashRegister', 'user', 'customer.person', 'creditAccount']);
            $this->auditLogger->logCreated($company, 'sale.created', $sale, $user);
