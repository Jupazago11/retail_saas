...
class UpdatePromotion
{
    ...
    public function handle(Company $company, Promotion $promotion, array $attributes): Promotion
    {
        if ((int) $promotion->company_id !== (int) $company->id) {
            throw new InvalidArgumentException('La promocion no pertenece a la empresa activa.');
        }
        ...
        return DB::transaction(function () use ($company, $promotion, $attributes, $promotionType, $discountType, $discountValue, $name, $targets, $comboItems) {
            $before = $promotion->fresh(['targets', 'comboItems']);
            ...
            $after = $promotion->fresh(['targets', 'comboItems']);
            $this->auditLogger->logUpdated($company, 'promotion.updated', $before, $after);
