    ...
    <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
        <div class="grid gap-4 md:grid-cols-4">
            <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-200">
                <p class="text-xs uppercase tracking-[0.18em] text-gray-500">Cuentas visibles</p>
            ...
            <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-200">
                <p class="text-xs uppercase tracking-[0.18em] text-gray-500">Movimientos</p>
                <p class="mt-2 text-3xl font-black text-blue-700">{{ $statusCards['movements_count'] }}</p>
            </div>
        </div>
        ...
        <div class="grid gap-6 xl:grid-cols-[0.95fr_1.15fr]">
            <div class="space-y-6">
                <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                        <div>
                            <p class="text-sm font-semibold uppercase tracking-[0.22em] text-blue-700">Programa</p>
                            <h3 class="mt-2 text-2xl font-black text-gray-900">Configuracion activa</h3>
                        </div>

                        <div class="grid gap-3 md:grid-cols-[170px_auto]">
                            <input wire:model="expirationAsOf" type="date" class="rounded-lg border-gray-300 shadow-sm focus:border-blue-600 focus:ring-blue-600">
                            <button wire:click="expirePoints" class="rounded-full bg-blue-600 px-4 py-2 text-sm font-semibold text-white">
                                Ejecutar expiracion
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="mt-5 grid gap-4 md:grid-cols-2">
                        <div class="rounded-xl bg-gray-50 p-4 ring-1 ring-gray-200">
                            <p class="text-xs uppercase tracking-[0.18em] text-gray-500">Estado</p>
                        ...
                        <div class="rounded-xl bg-gray-50 p-4 ring-1 ring-gray-200">
                            <p class="text-xs uppercase tracking-[0.18em] text-gray-500">Regla</p>
                            <p class="mt-2 text-lg font-black text-gray-900">{{ $this->ruleTypeLabel($settingsSnapshot['rule_type']) }}</p>
                        </div>
                        <div class="rounded-xl bg-gray-50 p-4 ring-1 ring-gray-200">
                            <p class="text-xs uppercase tracking-[0.18em] text-gray-500">Tasa</p>
                            <p class="mt-2 text-lg font-black text-gray-900">{{ $settingsSnapshot['points_rate'] }}</p>
                        </div>
                        <div class="rounded-xl bg-gray-50 p-4 ring-1 ring-gray-200">
                            <p class="text-xs uppercase tracking-[0.18em] text-gray-500">Expiracion</p>
                ...
                <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                        <div>
                            <p class="text-sm font-semibold uppercase tracking-[0.22em] text-blue-700">Clientes</p>
                            <h3 class="mt-2 text-2xl font-black text-gray-900">Cuentas de puntos</h3>
                        </div>

                        <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_180px]">
                            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Buscar por cliente o documento" class="rounded-lg border-gray-300 shadow-sm focus:border-blue-600 focus:ring-blue-600">
                            ...
                            <div class="rounded-xl border border-gray-200 bg-gray-50 p-5">
                                <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                                    <div class="space-y-2">
                                        <div class="flex flex-wrap items-center gap-2">
                                            <h4 class="text-lg font-black text-gray-900">{{ $customerName !== '' ? $customerName : 'Cliente '.$account->customer_id }}</h4>
                                            <x-status-badge :color="$account->status === 'active' ? 'emerald' : ($account->status === 'blocked' ? 'amber' : 'stone')">
                                                {{ $this->statusLabel($account->status) }}
                                            </x-status-badge>
                                        </div>

                                        <div class="grid gap-2 text-sm text-gray-600 md:grid-cols-2">
                                            <p>Documento: <span class="font-medium text-gray-900">{{ $account->customer?->person?->document_number ?: 'Sin documento' }}</span></p>
                                            <p>Movimientos: <span class="font-medium text-gray-900">{{ $account->movements->count() }}</span></p>
                                        </div>
                                    </div>

                                    <div class="space-y-3">
                                        <div class="rounded-lg bg-white p-4 ring-1 ring-gray-200">
                                            <p class="text-xs uppercase tracking-[0.18em] text-gray-500">Saldo actual</p>
                                            <p class="mt-1 text-xl font-black text-gray-900">{{ number_format((float) $account->points_balance, 4, '.', ',') }}</p>
                                        </div>

                                        <button wire:click="startAdjustingAccount({{ $account->id }})" class="rounded-full bg-blue-600 px-4 py-2 text-sm font-semibold text-white">
                                            Ajustar puntos
                                        </button>
                                    </div>
                                </div>
                                    ...
                                    <div class="mt-4 rounded-lg border border-sky-200 bg-sky-50 p-4">
                                        <div class="space-y-4">
                                            ...
                                            <div>
                                                ...
                                                <input wire:model="adjustmentNotes" type="text" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-sky-500 focus:ring-sky-500">
                                            ...
                                            <div class="flex gap-2">
                                                <button wire:click="applyAdjustment" class="rounded-full bg-blue-600 px-4 py-2 text-sm font-semibold text-white">
                                                    Aplicar ajuste
                                                </button>
                                                <button wire:click="cancelAdjustment" class="rounded-full border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700">
                                                    Cancelar
                                                </button>
                                            </div>
                                        </div>
                                ...
                                @if ($account->movements->isNotEmpty())
                                    <div class="mt-4 rounded-lg bg-white p-4 ring-1 ring-gray-200">
                                        <p class="text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">Ultimos movimientos</p>
                                        <div class="mt-3 space-y-2">
                                            @foreach ($account->movements->take(3) as $movement)
                                                ...
                                                <div class="flex flex-col gap-1 text-sm text-gray-600">
                                                    <p>
                                                        <span class="font-medium text-gray-900">{{ $this->movementLabel($movement->movement_type) }}</span>
            ...
            <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        <p class="text-sm font-semibold uppercase tracking-[0.22em] text-blue-700">Ledger</p>
                        <h3 class="mt-2 text-2xl font-black text-gray-900">Movimientos recientes</h3>
                    </div>
                    ...
                    @forelse ($recentMovements as $movement)
                        ...
                        <div class="rounded-xl border border-gray-200 bg-gray-50 p-5">
                            <div class="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                                <div class="space-y-2">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <h4 class="text-lg font-black text-gray-900">{{ $this->movementLabel($movement->movement_type) }}</h4>
                                        <x-status-badge :color="$direction >= 0 ? 'emerald' : 'rose'">
                                            {{ $direction >= 0 ? 'Entrada' : 'Salida' }}
                                        </x-status-badge>
                                    </div>

                                    <div class="grid gap-2 text-sm text-gray-600 md:grid-cols-2">
                                        <p>Cliente: <span class="font-medium text-gray-900">{{ $customerName !== '' ? $customerName : 'Cliente '.$movement->loyaltyAccount?->customer_id }}</span></p>
                                        <p>Venta: <span class="font-medium text-gray-900">{{ $movement->sale_id ? '#'.$movement->sale_id : 'N/A' }}</span></p>
                                        ...
                                        <p>Saldo despues: <span class="font-medium text-gray-900">{{ number_format((float) $movement->balance_after, 4, '.', ',') }}</span></p>
                                        <p>Efectivo equivalente: <span class="font-medium text-gray-900">${{ \App\Support\Money::format((float) $movement->cash_equivalent) }}</span></p>
                                        <p>Fecha: <span class="font-medium text-gray-900">{{ optional($movement->occurred_at)->format('Y-m-d H:i') ?: 'Sin fecha' }}</span></p>
                                        @if ($movement->notes)
                                            <p>Notas: <span class="font-medium text-gray-900">{{ $movement->notes }}</span></p>
                                        @endif
                                    </div>
                                </div>
                            </div>
